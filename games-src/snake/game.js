  ...
  addScore() {
    ...
    if(table) {
      if(score > table.score) {
        table.score = score
        this.parent.scores[username] = table
      }
    } else {
      this.parent.scores[username] = {username: username, score: score}
    }

    // Persist to localStorage and refresh the rendered table.
    let list = []
    for (let key in this.parent.scores) {
      if (Object.prototype.hasOwnProperty.call(this.parent.scores, key)) {
        list.push(this.parent.scores[key])
      }
    }
    try {
      localStorage.setItem("snack-snake-highscores", JSON.stringify(list))
    } catch (e) { /* storage unavailable — scores stay in memory */ }
    this.parent.highscoreList()
  }
