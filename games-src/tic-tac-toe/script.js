...
function makeMove(index, symbol) {
    ...
    cell.innerHTML = `<span class="icon-${symbol}" aria-hidden="true">${symbol.toUpperCase()}</span>`;
...
function updateTurnIndicator() {
    turnIcon.className = `icon-${currentPlayer}`; turnIcon.textContent = currentPlayer.toUpperCase();
}
...
function showResult(isDraw, winner) {
    if (isDraw) {
        ...
        modalWinnerAnnouncement.innerHTML = `
            <span class="icon-winner icon-${winner}" aria-hidden="true">${winner.toUpperCase()}</span>
